...
for vid=1:num_vid
    ...
    names = strsplit(testlist{vid},'/');
    names = names{2};
    [~,dtVidInd] = find(strcmp(dtNames,names));
...
%                 end
                ...
                iou = compute_spatio_temporal_iou_bmvc(gt_fnr, gt_bb, dt_fnr, dt_bb);
                if iou>ioumax
                    ioumax=iou;
                    maxgtind=gtind;
                end
...
for a=1:num_actions
    ...
    precision=tp./(fp+tp);
    AP(a) = xVOCap_bmvc(recall,precision);
    draw = 0;
    if draw
